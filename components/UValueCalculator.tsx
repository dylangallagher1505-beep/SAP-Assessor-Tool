  ...
  return (
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div style={{ borderTop: '4px solid #16a34a', paddingTop: '1rem' }}>
        <h1 className="text-3xl font-black" style={{ color: '#14532d' }}>U-VALUE CALCULATOR</h1>
        <p className="text-sm mt-1 font-medium" style={{ color: '#64748b' }}>
          BR443:2019 combined method · SAP 10.2
        </p>
      </div>
        ...
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-48">
            ...
            <select
              ...
              style={{ border: '2px solid #e2e8f0', background: 'white', color: '#14532d' }}
            >
              {ELEMENT_TYPES.map((t) => (
      ...
      <div className="flex gap-6 items-start">
        ...
        <div className="w-72 shrink-0 space-y-4">
          ...
          >
            ...
            <div className="divide-y" style={{ borderColor: '#f8fafc' }}>
              ...
              {result.layers.map((lr) => (
                <RRow
                  key={lr.id}
                  label={lr.name || 'Unnamed layer'}
                  value={lr.R}
                  sub={lr.bridgingDetails}
                />
              ))}
          ...
          {savedMessage && (
            <p className="text-xs text-center font-bold" style={{ color: '#16a34a' }}>{savedMessage}</p>
          )}
        </div>
      </div>
    </div>
...
function SurfaceRow({ label, value }: { label: string; value: number }) {
  return (
    ...
    >
      ...
      <span className="text-sm font-mono font-semibold w-24 text-right" style={{ color: '#94a3b8' }}>
        {value.toFixed(2)} m²K/W
      </span>
    </div>
  );
}
...
function RRow({ label, value, sub, bold }: { label: string; value: number; sub?: string; bold?: boolean }) {
  return (
    <div
      className="flex items-start px-4 py-2 gap-2"
      style={bold ? { background: '#f8fafc', fontWeight: 700 } : {}}
    >
      <span className="flex-1 text-xs leading-snug" style={{ color: '#475569' }}>
        {label}
        {sub && <span className="block font-normal" style={{ color: '#94a3b8' }}>{sub}</span>}
      </span>
      ...
      <div className="flex items-start gap-3 p-4">
        ...
        <div className="flex flex-col gap-0.5 pt-1 shrink-0">
          <button
            onClick={() => onMove(-1)}
            disabled={index === 0}
            className="w-6 h-6 flex items-center justify-center rounded hover:bg-slate-100 disabled:opacity-20 text-xs"
            style={{ color: '#94a3b8' }}
          >▲</button>
          <button
            onClick={() => onMove(1)}
        ...
        {/* Colour swatch */}
        <div className="w-5 h-5 rounded shrink-0 mt-2" style={{ background: '#4ade80' }} />

        {/* Material selector */}
        <div className="flex-1 relative min-w-0">
          <div className="text-xs font-black uppercase tracking-widest mb-1" style={{ color: '#94a3b8' }}>
            Material
          </div>
          <button
            type="button"
            ...
            style={{
              ...
              background: 'white',
              color: layer.name ? '#14532d' : '#94a3b8',
            }}
          >
          ...
          {showDropdown && (
            <div
              className="absolute z-50 top-full left-0 mt-1 rounded-2xl shadow-2xl overflow-hidden"
              style={{ width: 'max(100%, 420px)', background: 'white', border: '2px solid #e2e8f0' }}
            >
              {/* Search */}
              <div className="p-2 sticky top-0" style={{ borderBottom: '1px solid #f1f5f9', background: 'white' }}>
                <input
                  ref={(el) => { el?.focus(); }}
                  type="text"
                  value={matSearch}
              ...
              <div className="max-h-72 overflow-y-auto">
                {/* Custom option */}
                <div
                  className="px-4 py-2.5 text-sm font-bold cursor-pointer flex items-center gap-2"
                  style={{ color: '#16a34a', borderBottom: '1px solid #f1f5f9' }}
                ...
                >
                  <span className="text-base">✏️</span> Custom — enter lambda manually
                </div>
                {grouped.length === 0 && (
                  <div className="px-4 py-6 text-sm text-center font-medium" style={{ color: '#94a3b8' }}>
                    No materials match &quot;{matSearch}&quot;
                  </div>
                )}
                {grouped.map((group) => (
                  <div key={group.category}>
                    <div
                      className="px-4 py-2 text-xs font-black uppercase tracking-widest sticky top-0"
                      style={{
                        color: '#64748b',
                        background: '#f8fafc',
                    ...
                    {group.items.map((mat) => (
                      <div
                        key={mat.id}
                        className="px-4 py-2.5 text-sm cursor-pointer flex items-center justify-between gap-4"
                        style={{ color: '#334155' }}
                        onMouseEnter={(e) => { (e.currentTarget as HTMLDivElement).style.background = '#f0fdf4'; }}
                        onMouseLeave={(e) => { (e.currentTarget as HTMLDivElement).style.background = ''; }}
                        onMouseDown={() => {
        ...
        <div className="w-24 shrink-0">
          ...
          <div
            ...
            style={{ border: '1px solid #f1f5f9', background: '#f8fafc', color: '#475569' }}
          >
            {hasFixed
        ...
        <button
          onClick={onRemove}
          className="mt-5 text-lg leading-none shrink-0 transition-colors"
          style={{ color: '#cbd5e1' }}
          title="Remove layer"
      ...
      <div className="px-4 pb-4 flex items-start gap-4">
        <label className="flex items-center gap-2 cursor-pointer select-none">
          ...
          <span className="text-xs font-medium" style={{ color: '#64748b' }}>
            Bridged layer (e.g. insulation between studs)
          </span>
        </label>
        ...
        {layer.bridged && !hasFixed && (
          <div className="flex gap-3 flex-wrap">
            <div>
              ...
              <input
                type="text"
                value={layer.bridgingMaterial?.name ?? ''}
                onChange={(e) =>
                  onUpdate({
                    bridgingMaterial: {
                      name: e.target.value,
                      lambda: layer.bridgingMaterial?.lambda ?? 0.13,
                    },
                  })
                }
                placeholder="e.g. Softwood studs"
                className="rounded-lg px-2 py-1.5 text-xs w-36 font-medium focus:outline-none"
                style={{ border: '2px solid #e2e8f0' }}
              />
            </div>
            <div>
              ...
              <input
                type="number"
                step="0.001"
                value={layer.bridgingMaterial?.lambda ?? ''}
                onChange={(e) =>
                  onUpdate({
                    bridgingMaterial: {
                      name: layer.bridgingMaterial?.name ?? '',
                      lambda: parseFloat(e.target.value) || 0,
                    },
                  })
                }
                placeholder="0.130"
                className="rounded-lg px-2 py-1.5 text-xs w-20 font-mono focus:outline-none"
                style={{ border: '2px solid #e2e8f0' }}
              />
            </div>
            <div>
