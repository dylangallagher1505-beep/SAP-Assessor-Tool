...
function saveToLibrary(c: SavedConstruction) {
  const existing = loadLibrary();
  localStorage.setItem('sap-construction-library', JSON.stringify([c, ...existing.filter((x) => x.id !== c.id)]));
}
...
export default function UValueCalculator() {
  ...
  const colourClass = { green: 'text-green-600 bg-green-50 border-green-200', amber: 'text-amber-600 bg-amber-50 border-amber-200', red: 'text-red-600 bg-red-50 border-red-200' }[uValueColour(result.uValue, elementType)];
    ...
    <div className="max-w-6xl mx-auto p-6 space-y-6">
      ...
      <div className="bg-white rounded-xl shadow-sm border border-slate-200 p-5">
        <div className="flex flex-wrap gap-4 items-end">
          <div className="flex-1 min-w-48">
            ...
            <select value={elementType} onChange={(e) => setElementType(e.target.value as ElementType)} className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm bg-white focus:outline-none focus:ring-2 focus:ring-blue-500">
          ...
          <div className="flex-1 min-w-48">
            ...
            <input type="text" value={constructionName} onChange={(e) => setConstructionName(e.target.value)} placeholder="e.g. External cavity wall — standard" className="w-full border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-blue-500" />
          </div>
        </div>
      </div>
      <div className="flex gap-6 items-start">
        <div className="flex-1 space-y-3">
          ...
          <button onClick={addLayer} className="w-full border-2 border-dashed border-slate-300 rounded-xl py-3 text-sm text-slate-500 hover:border-blue-400 hover:text-blue-600 transition-colors font-medium">+ Add Layer</button>
        </div>
        <div className="w-72 shrink-0 space-y-4">
          ...
          <div className="bg-white rounded-xl border border-slate-200 shadow-sm overflow-hidden">
            <div className="px-4 py-3 border-b border-slate-100"><span className="text-xs font-semibold text-slate-500 uppercase tracking-wide">R Breakdown</span></div>
          ...
          <button onClick={handleSave} className="w-full bg-blue-600 hover:bg-blue-700 text-white rounded-xl py-3 text-sm font-semibold transition-colors">Save to Library</button>
          {savedMessage && <p className="text-xs text-green-600 text-center font-medium">{savedMessage}</p>}
        </div>
      </div>
    </div>
  );
...
function RRow({ label, value, sub, bold }: { label: string; value: number; sub?: string; bold?: boolean }) {
  return (
    <div className={`flex items-start px-4 py-2 gap-2 ${bold ? 'bg-slate-50 font-semibold' : ''}`}>
      <span className="flex-1 text-xs text-slate-600 leading-snug">{label}{sub && <span className="block text-slate-400 font-normal">{sub}</span>}</span>
    ...
    <div className="bg-white border border-slate-200 rounded-xl shadow-sm overflow-visible">
      <div className="flex items-start gap-3 p-4">
        <div className="flex flex-col gap-0.5 pt-1 shrink-0">
          <button onClick={() => onMove(-1)} disabled={index === 0} className="w-6 h-6 flex items-center justify-center rounded hover:bg-slate-100 disabled:opacity-20 text-slate-400 text-xs">▲</button>
          <button onClick={() => onMove(1)} disabled={index === total - 1} className="w-6 h-6 flex items-center justify-center rounded hover:bg-slate-100 disabled:opacity-20 text-slate-400 text-xs">▼</button>
        </div>
        ...
        <div className="flex-1 relative min-w-0">
          ...
          <input type="text" value={matSearch || layer.name}
            onFocus={() => { setMatSearch(''); setShowDropdown(true); }}
            onChange={(e) => { setMatSearch(e.target.value); setShowDropdown(true); }}
          ...
          {showDropdown && (
            <div className="absolute z-50 top-full left-0 right-0 mt-1 bg-white border border-slate-200 rounded-xl shadow-xl max-h-64 overflow-y-auto">
              <div className="px-3 py-2 text-xs text-blue-600 font-medium cursor-pointer hover:bg-blue-50 border-b border-slate-100"
                onMouseDown={() => { onUpdate({ name: 'Custom', lambda: 0.04, fixedR: undefined }); setShowDropdown(false); setMatSearch(''); }}>
                Custom — enter lambda manually
              </div>
              {grouped.map((group) => (
                <div key={group.category}>
                  <div className="px-3 py-1.5 text-xs font-semibold text-slate-400 uppercase tracking-wide bg-slate-50">{group.category}</div>
                  {group.items.map((mat) => (
                    <div key={mat.id} className="px-3 py-2 text-sm hover:bg-blue-50 cursor-pointer flex justify-between items-center"
                      onMouseDown={() => { onSelectMaterial(mat); setShowDropdown(false); setMatSearch(''); }}>
                      <span className="text-slate-700">{mat.name}</span>
                      <span className="text-xs text-slate-400 font-mono ml-2 shrink-0">{mat.fixedR ? `R=${mat.fixedR}` : `λ=${mat.lambda}`}</span>
                    </div>
                  ))}
                </div>
              ))}
        ...
        <button onClick={onRemove} className="mt-5 text-slate-300 hover:text-red-500 transition-colors text-lg leading-none shrink-0" title="Remove layer">✕</button>
      </div>
