...
export default function OpeningsPanel() {
  ...
  if (!story || story.walls.length === 0) {
    return (
      <div className="text-xs text-slate-500 italic px-1 pt-1">
  ...
  return (
    <div className="flex flex-col gap-1">
      {story.walls.map((wall, i) => {
        ...
        return (
          <div key={wall.id} className="border border-slate-700 rounded-lg overflow-hidden">
            {/* Wall header */}
            <button
              className="w-full flex items-center gap-1.5 px-2 py-1.5 text-left hover:bg-slate-800/60 text-xs"
              onClick={() => setExpandedWall(isOpen ? null : wall.id)}
            >
              {isOpen ? <ChevronDown size={11} /> : <ChevronRight size={11} />}
              <span className="text-slate-300 flex-1">{wallLabel(i, len)}</span>
              <span className="text-slate-500">{wallOpenings.length > 0 ? `${wallOpenings.length} opening${wallOpenings.length > 1 ? 's' : ''}` : ''}</span>
            </button>

            {isOpen && (
              <div className="px-2 pb-2 bg-slate-900/60 flex flex-col gap-2">
                {/* Existing openings */}
                {wallOpenings.map((op) => (
                  <OpeningEditor
                    key={op.id}
...
function OpeningEditor({
  ...
  return (
    <div className="bg-slate-800 rounded p-2 flex flex-col gap-1.5 text-xs">
      {/* Type + remove */}
      <div className="flex items-center gap-2">
        <select
          value={opening.type}
          onChange={(e) => onUpdate({ type: e.target.value as OpeningType })}
          className="flex-1 bg-slate-700 border border-slate-600 rounded px-1.5 py-0.5 text-slate-200 focus:outline-none"
        >
          <option value="window">Window</option>
          <option value="door">Door</option>
        </select>
        <button onClick={onRemove} className="text-slate-500 hover:text-red-400"><Trash2 size={11} /></button>
      </div>

      {/* Position along wall */}
      <div className="flex items-center gap-2">
        <label className="text-slate-500 w-14 shrink-0">Position</label>
        <input
        ...
        />
        <span className="text-slate-400 w-10 text-right">{(opening.uOffset * wallLen).toFixed(1)}m</span>
      </div>

      {/* Width */}
      <div className="flex items-center gap-2">
        <label className="text-slate-500 w-14 shrink-0">Width</label>
        <input
          ...
          className="flex-1 accent-blue-500"
        />
        <span className="text-slate-400 w-10 text-right">{opening.width.toFixed(2)}m</span>
      </div>

      {/* Height */}
      <div className="flex items-center gap-2">
        <label className="text-slate-500 w-14 shrink-0">Height</label>
        <input
          ...
          className="flex-1 accent-blue-500"
        />
        <span className="text-slate-400 w-10 text-right">{opening.height.toFixed(2)}m</span>
      </div>

      {/* Sill height */}
      {opening.type === 'window' && (
        <div className="flex items-center gap-2">
          <label className="text-slate-500 w-14 shrink-0">Sill</label>
          <input
            ...
            className="flex-1 accent-blue-500"
          />
          <span className="text-slate-400 w-10 text-right">{opening.sillHeight.toFixed(2)}m</span>
        </div>
      )}

      {/* U-value */}
      <div className="flex items-center gap-2">
        <label className="text-slate-500 w-14 shrink-0">U-value</label>
        <input
          type="number" step={0.1} min={0.5} max={5}
          value={opening.uValue}
          onChange={(e) => onUpdate({ uValue: parseFloat(e.target.value) || 1.4 })}
          className="w-16 bg-slate-700 border border-slate-600 rounded px-1.5 py-0.5 text-slate-200 focus:outline-none"
        />
