  ...
  return (
    <div className="max-w-5xl mx-auto p-6 space-y-6">
      <div style={{ borderTop: '4px solid #FFD700', paddingTop: '1rem' }}>
        <h1 className="text-3xl font-black" style={{ color: '#0f1729' }}>Construction Library</h1>
        <p className="text-sm mt-1 font-medium" style={{ color: '#64748b' }}>Saved constructions from the U-Value Calculator</p>
      </div>

      {constructions.length === 0 && (
        <div className="rounded-2xl p-12 text-center" style={{ background: 'white', border: '2px solid #e2e8f0' }}>
          <div className="text-5xl mb-4">📐</div>
          <div className="text-lg font-black mb-1" style={{ color: '#0f1729' }}>No constructions saved yet</div>
      ...
      <div className="grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
        {constructions.map((c) => {
          ...
          return (
            <div key={c.id} className="rounded-2xl overflow-hidden flex flex-col transition-all"
              ...
              <div className="p-5 flex-1">
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h3 className="font-black text-base leading-snug" style={{ color: '#0f1729' }}>{c.name}</h3>
                  <span className="text-xs font-black px-2.5 py-1 rounded-full shrink-0" style={badgeStyle}>{c.uValue.toFixed(3)}</span>
                </div>
                <div className="text-xs font-semibold mb-3" style={{ color: '#64748b' }}>{ELEMENT_TYPE_LABELS[c.elementType]}</div>
                <div className="text-xs font-medium" style={{ color: '#94a3b8' }}>{c.layers.length} layer{c.layers.length !== 1 ? 's' : ''} · R = {c.totalR.toFixed(3)} m²K/W</div>
                {isExpanded && (
                  <div className="mt-3 pt-3 space-y-1.5" style={{ borderTop: '1px solid #f1f5f9' }}>
                    {c.layers.map((l, i) => (
                      <div key={i} className="flex justify-between text-xs gap-2">
                        <span className="flex-1 truncate font-medium" style={{ color: '#475569' }}>{l.name}</span>
                        <span className="font-mono shrink-0" style={{ color: '#94a3b8' }}>
                          {l.fixedR !== undefined ? `R=${l.fixedR}` : `${l.thicknessMm}mm / λ${l.lambda?.toFixed(3)}`}
                        </span>
                      </div>
                    ))}
                  </div>
              ...
              <div className="px-5 pb-5 flex gap-2">
                <button onClick={() => setExpanded(isExpanded ? null : c.id)}
                  ...
                  onMouseEnter={(e) => { const b = e.currentTarget; b.style.background = '#f8fafc'; b.style.borderColor = '#94a3b8'; }}
                  onMouseLeave={(e) => { const b = e.currentTarget; b.style.background = 'white'; b.style.borderColor = '#e2e8f0'; }}>
              ...
              <div className="px-5 pb-3 text-xs font-medium" style={{ color: '#cbd5e1' }}>Saved {new Date(c.savedAt).toLocaleDateString('en-GB')}</div>
            </div>
          );
        })}
      </div>
